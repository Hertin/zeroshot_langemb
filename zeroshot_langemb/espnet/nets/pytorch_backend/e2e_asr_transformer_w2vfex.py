...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.lang2ph is not None:
            ...
            for lang, phones in self.lang2ph.items(): 
                self.lang2phid[lang] = list(map(self.args.char_list.index, phones+['<blank>', '<unk>', '<space>', '<eos>']))
    ...
    def forward(self, audio_pad, audio_lens, ys_pad):
        """E2E forward.
        ...
        hs_pad, hs_mask = self.encoder(features, src_mask)
    ...
    def encode_with_length(self, audio_pad, audio_lens, lang_labels=None, **kwargs):
        """Encode acoustic features.
        ...
        # audio_pad_mask = make_pad_mask(audio_lens)
        # hs_pad, hs_mask = self.encoder(audio_pad, audio_pad_mask)
        ...
        hs_len = hs_mask.view(batch_size, -1).sum(1)
        log_probs = self.ctc.log_softmax(hs_pad.view(batch_size, -1, self.adim), lang_labels=lang_labels)
